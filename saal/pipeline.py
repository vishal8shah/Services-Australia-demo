...
def answer(question: str, *, conn=None, provider=None, embedder=None,
    ...
    if conn is None:
        with store.connect() as owned_conn:
            return _retrieve_and_answer(owned_conn, question, facet_list, lang, out,
                                        finish, provider, embedder, top_k)
    return _retrieve_and_answer(conn, question, facet_list, lang, out, finish,
                                provider, embedder, top_k)


def _retrieve_and_answer(conn, question, facet_list, lang, out, finish,
                         provider, embedder, top_k):
    hits = search(conn, question, facets=facet_list, top_k=top_k, embedder=embedder)
    ...
    if not hits or confidence < config.SCORE_FLOOR:
        return finish(refuse.build("low_confidence",
                      ...
                      hits=len(hits), retrieved=[h.chunk.chunk_id for h in hits])
...
def _assemble(out, result, hits, finish, provider, attempts):
